...
export default function EditarCuentaPage() {
  ...
  const handleUpdateInfo = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(error);
      setInfoMessage({ type: 'error', text: 'Error al actualizar la información.' });
    } finally {
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.code === 'auth/invalid-credential' || error.code === 'auth/wrong-password') {
        ...
        setPasswordMessage({ type: 'error', text: 'Error al actualizar la contraseña. Re-autenticación podría ser necesaria.' });
      }
    } finally {
  ...
  const handleDeleteAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(error);
      if (error.code === 'auth/invalid-credential' || error.code === 'auth/wrong-password') {
        setDeleteMessage({ type: 'error', text: 'La contraseña es incorrecta.' });
      } else {
        setDeleteMessage({ type: 'error', text: 'Error al eliminar la cuenta. Por favor intente nuevamente.' });
      }
    } finally {
      setIsDeleting(false);
    }
  ...
  if (authLoading || !user) {
    return <div className="p-12 text-center text-slate-500 text-sm">Cargando...</div>;
  }
    ...
    <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8 space-y-8">
      ...
      <div className="flex items-center gap-4 border-b border-slate-200 pb-5">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Gestione su información personal y de seguridad</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs">
          ...
          <form onSubmit={handleUpdateInfo} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">Dirección Completa</label>
              <textarea
                value={address}
                ...
                placeholder="Ej. Colonia Universidad, 3ra calle, casa #12"
              />
            </div>
      ...
      <div className="bg-red-50 border border-red-200 rounded-2xl p-6 shadow-xs mt-8">
        <h2 className="text-lg font-bold text-red-700 flex items-center gap-2 mb-2">
          <AlertTriangle className="w-5 h-5 text-red-600" /> Advertencia: Eliminar Cuenta
        </h2>
