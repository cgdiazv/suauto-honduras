          ...
          <tbody className="divide-y divide-slate-100">
            {vehicles.map((car) => (
              <tr key={car.id} className="hover:bg-slate-50/50">
                ...
                <td className="p-4">
                  <span className={`text-[10px] sm:text-xs px-2 py-0.5 rounded-full border ${car.status === 'Disponible' ? 'bg-[#67bd45] text-white border-[#67bd45]' : car.status === 'Reservado' ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-slate-50 text-slate-700 border-slate-200'}`}>{car.status}</span>
                </td>
